...
#include "msarray.h"         // Double-inclusion check, for testing only
...
const size_t DEFAULTSIZE = 8;  // Size of default-constructed MSArray<T>
...
TEST_CASE( "Function return types are correct" )
{
    ...
    SUBCASE( "Copy assn returns MSArray by reference" )
    {
        [[maybe_unused]] MSArray<int> & r(mi1 = cmi2);
        REQUIRE( TypeCheck<MSArray<int>>::check(mi1 = cmi2) );
    }

    SUBCASE( "Move assn returns MSArray by reference" )
    {
        MSArray<int> mi_movable1;
        MSArray<int> mi_movable2;
        [[maybe_unused]] MSArray<int> & r(mi1 = move(mi_movable1));
    ...
    SUBCASE( "Bracket op (const) returns item by ref-to-const" )
    {
        ...
        [[maybe_unused]] const MSArray<int>::value_type & r2(cmi2[0]);
        ...
        [[maybe_unused]] const MSArray<double>::value_type & r4(cmd5[0]);
    ...
    SUBCASE( "size returns size_type by value" )
    {
        ...
        [[maybe_unused]] MSArray<int>::size_type && r2(cmi2.size());
        REQUIRE( TypeCheck<MSArray<int>::size_type>::check(cmi2.size()) );

        [[maybe_unused]] size_t && r3(cmd5.size());
        [[maybe_unused]] MSArray<int>::size_type && r4(cmd5.size());
    ...
    SUBCASE( "begin (const) returns ptr-to-item by value" )
    {
        ...
        [[maybe_unused]] const MSArray<int>::value_type * && r2(cmi2.begin());
        ...
        [[maybe_unused]] const MSArray<double>::value_type * && r4(cmd5.begin());
    ...
    SUBCASE( "begin (non-const) returns ptr-to-item by value" )
    {
        ...
        [[maybe_unused]] MSArray<int>::value_type * && r2(mi1.begin());
        ...
        [[maybe_unused]] MSArray<double>::value_type * && r4(md4.begin());
    ...
    SUBCASE( "end (const) returns ptr-to-item by value" )
    {
        ...
        [[maybe_unused]] const MSArray<int>::value_type * && r2(cmi2.end());
        ...
        [[maybe_unused]] const MSArray<double>::value_type * && r4(cmd5.end());
    ...
    SUBCASE( "end (non-const) returns ptr-to-item by value" )
    {
        ...
        [[maybe_unused]] MSArray<int>::value_type * && r2(mi1.end());
        ...
        [[maybe_unused]] MSArray<double>::value_type * && r4(md4.end());
...
{
    SUBCASE( "Taking const MSArray arguments" )
    {
        ...
        [[maybe_unused]] bool result1 = (cmi1 == cmi2);  // op==(MSArray,MSArray)
        [[maybe_unused]] bool result2 = (cmi1 != cmi2);  // op!=(MSArray,MSArray)
        [[maybe_unused]] bool result3 = (cmi1 <  cmi2);  // op<(MSArray,MSArray)
        [[maybe_unused]] bool result4 = (cmi1 <= cmi2);  // op<=(MSArray,MSArray)
        [[maybe_unused]] bool result5 = (cmi1 >  cmi2);  // op>(MSArray,MSArray)
        [[maybe_unused]] bool result6 = (cmi1 >= cmi2);  // op>=(MSArray,MSArray)
...
TEST_CASE( "Comparisons allow min-functionality value types (passes if it compiles)" )
{
    ...
    {
        const MSArray<OnlyEq> cmo1(2);
        const MSArray<OnlyEq> cmo2(2);
...
TEST_CASE( "Checks for noexcept" )
{
    ...
    {
        REQUIRE( noexcept(MSArray<int>(move(mi1))) );
    }
    ...
    {
        REQUIRE( noexcept(mi1.~MSArray()) );
    }

    SUBCASE( "Copy ctor is NOT noexcept" )
    {
        REQUIRE_FALSE( noexcept(MSArray<int>(mi1)) );
    }
    ...
    {
        REQUIRE_FALSE( noexcept(MSArray<int>()) );
    }

    SUBCASE( "Ctor from size is NOT noexcept" )
    {
        REQUIRE_FALSE( noexcept(MSArray<int>(size_t(100))) );
    }
}
...
TEST_CASE( "Size & ctor from size+value" )
{
    ...
    {
        const MSArray<int> cmi(1000000, -63);
...
TEST_CASE( "Ctor/dctor/assignment count" )
{
   ...
   Counter::reset();               // Restart all counts
    ...
    {
        {  // Block, so we get dctor calls before function ends
            const MSArray<Counter> mc1(thesize);
    ...
    SUBCASE( "Copy ctor: value-type ctor/dctor/assignment counts" )
    {
        {  // Block, so we get dctor calls before function ends
            ...
            const MSArray<Counter> mc2(mc1);  // Copy ctor
    ...
    SUBCASE( "Move ctor: value-type ctor/dctor/assignment counts" )
    {
        {  // Block, so we get dctor calls before function ends
            ...
            const MSArray<Counter> mc2(move(mc1));  // Move ctor
